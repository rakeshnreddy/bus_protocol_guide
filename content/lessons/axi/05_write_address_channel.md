...
*   **`AWLEN`** (Write Burst Length): Defines how many data beats are in the burst. In AXI4, this is an 8-bit signal allowing up to 256 beats (`AWLEN` = Number of beats - 1. So `AWLEN=0` means 1 beat).
...
*   **`AWLOCK`**: Used for exclusive accesses (atomic operations).
...
*   **`AWQOS`**: (AXI4 only) Quality of Service identifier to prioritize traffic in interconnects.
*   **`AWREGION`**: (AXI4 only) Region identifier to simplify slave decoding.
